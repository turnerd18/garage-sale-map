...
export default async function handler(request: Request): Promise<Response> {
  ...
  if (request.method === "GET") {
    ...
    const result = await sqlite.execute({
      sql: `SELECT id, street_address, city, state, zip, name, last_updated_ip_address, is_closed, created_at, updated_at FROM locations ${includeClosed ? "" : "WHERE is_closed = 0"} ORDER BY is_closed ASC, updated_at DESC, created_at DESC`
    });
  ...
  if (request.method === "POST") {
    ...
    const result = await sqlite.execute({
      sql: `INSERT INTO locations (street_address, city, state, zip, name, last_updated_ip_address, is_closed, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 0, ?, ?) RETURNING id, name`,
      args: [body.street_address, body.city, body.state, body.zip, name, ip, now, now]
    });
