...
function milesBetween([lat1, lng1], [lat2, lng2]) { const r=(d)=>d*Math.PI/180; const R=3958.8; const dLat=r(lat2-lat1); const dLng=r(lng2-lng1); const a=Math.sin(dLat/2)**2+Math.cos(r(lat1))*Math.cos(r(lat2))*Math.sin(dLng/2)**2; return 2*R*Math.asin(Math.sqrt(a)); }
function FitSalesBounds({ sales }) { const map = useMap(); useEffect(() => { if (!sales.length) return; map.fitBounds(L.latLngBounds(sales.map((s) => [s.lat, s.lng])).pad(0.15)); }, [map, sales]); return null; }

function App() {
  const [sales, setSales] = useState([]); const [error, setError] = useState(''); const [loading, setLoading] = useState(true);
  const [view, setView] = useState('map'); const [includeClosed, setIncludeClosed] = useState(false); const [showAdd, setShowAdd] = useState(false);
  const [form, setForm] = useState({ street_address: '', city: 'Ottawa', state: 'KS', zip: '', name: '', lat: '', lng: '' });
  const visibleSales = useMemo(() => sales.filter((s) => includeClosed || !s.is_closed), [sales, includeClosed]);
  const fetchSales = async () => { try { setLoading(true); const response = await fetch(`${VAL_TOWN_ENDPOINT}?includeClosed=true`); if (!response.ok) throw new Error(`Request failed: ${response.status}`); const payload = await response.json(); const list = Array.isArray(payload) ? payload : payload.sales ?? []; const cleaned = list.map((item, index) => ({ id: item.id ?? `${item.street_address}-${index}`, address: [item.street_address, item.city, item.state, item.zip].filter(Boolean).join(', '), street_address: item.street_address, name: item.name ?? 'Garage Sale', lat: Number(item.lat), lng: Number(item.lng), is_closed: Boolean(item.is_closed) })).filter((i) => i.street_address && !Number.isNaN(i.lat) && !Number.isNaN(i.lng) && milesBetween(OTTAWA_CENTER, [i.lat, i.lng]) <= MAX_DISTANCE_MILES); setSales(cleaned); setError(''); } catch (e) { setError(`Unable to load sales. ${e.message}`); } finally { setLoading(false); } };
  useEffect(() => { fetchSales(); }, []);
  const addSale = async (event) => { event.preventDefault(); const res = await fetch(VAL_TOWN_ENDPOINT, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(form) }); if (!res.ok) { const body = await res.json().catch(() => ({})); setError(body.error ?? 'Failed to add sale'); return; } setShowAdd(false); setForm({ street_address: '', city: 'Ottawa', state: 'KS', zip: '', name: '', lat: '', lng: '' }); fetchSales(); };
  const markClosed = async (id, isClosed) => { const res = await fetch(VAL_TOWN_ENDPOINT, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ id, is_closed: isClosed }) }); if (!res.ok) { const body = await res.json().catch(() => ({})); setError(body.error ?? 'Failed to update sale'); return; } fetchSales(); };
