...
function App() {
  ...
  const fetchSales = async () => {
    try {
      ...
      })).filter((i) => i.street_address && !Number.isNaN(i.lat) && !Number.isNaN(i.lng) && milesBetween(OTTAWA_CENTER, [i.lat, i.lng]) <= MAX_DISTANCE_MILES);
      setSales(cleaned); setError('');
    } catch (e) { setError(`Unable to load sales. ${e.message}`); }
  ...
  const addSale = async (event) => {
    ...
    const res = await fetch(VAL_TOWN_ENDPOINT, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(form) });
    if (!res.ok) { const body = await res.json().catch(() => ({})); setError(body.error ?? 'Failed to add sale'); return; }
    setShowAdd(false); setForm({ street_address: '', city: 'Ottawa', state: 'KS', zip: '', name: '', lat: '', lng: '' }); fetchSales();
  };

  const markClosed = async (id, isClosed) => {
    const res = await fetch(VAL_TOWN_ENDPOINT, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ id, is_closed: isClosed }) });
    if (!res.ok) { const body = await res.json().catch(() => ({})); setError(body.error ?? 'Failed to update sale'); return; }
    fetchSales();
  };
